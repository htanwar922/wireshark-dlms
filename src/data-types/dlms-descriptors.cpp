...
dlms_dissect_cosem_attribute_or_method_descriptor(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gint *offset, int is_attribute)
{
    ...
    item = proto_tree_add_item(subtree, *dlms_hdr.class_id.p_id, tvb, *offset, 2, ENC_BIG_ENDIAN);
    if (cosem_class) {
        proto_item_append_text(item, ": %s (%u)", cosem_class->name, class_id);
    } else {
        proto_item_append_text(item, ": Unknown (%u)", class_id);
        expert_add_info(pinfo, item, dlms_ei.not_implemented.ids);
    }
    *offset += 2;

    item = proto_tree_add_item(subtree, *dlms_hdr.instance_id.p_id, tvb, *offset, 6, ENC_NA);
    ...
    *offset += 6;
    ...
    item = proto_tree_add_item(subtree,
                               is_attribute ? *dlms_hdr.attribute_id.p_id : *dlms_hdr.method_id.p_id,
