...
dlms_dissect_compact_array_content(tvbuff_t *tvb, proto_tree *tree, gint description_offset, gint *content_offset)
{
    ...
    item = proto_tree_add_item(tree, *dlms_hdr.data.p_id, tvb, *content_offset, 0, ENC_NA);
...
dlms_dissect_data(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gint *offset)
{
    ...
    item = proto_tree_add_item(tree, *dlms_hdr.data.p_id, tvb, *offset, 1, ENC_NA);
    ...
    *offset += 1;
    if (choice == 1) { /* array */
        ...
        unsigned elements;
        subtree = proto_item_add_subtree(item, dlms_ett.composite_data);
        proto_tree_add_item(subtree, *dlms_hdr.type_description.p_id, tvb, description_offset, description_length, ENC_NA);
...
dlms_dissect_datablock_data(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, proto_tree *subtree, gint *offset, unsigned block_number, unsigned last_block)
{
    ...
    item = proto_tree_add_item(subtree, *dlms_hdr.data.p_id, tvb, saved_offset, *offset - saved_offset + raw_data_length, ENC_NA);
...
dlms_dissect_datablock_g(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gint *offset)
{
    ...
    proto_tree_add_item(subtree, *dlms_hdr.last_block.p_id, tvb, *offset, 1, ENC_NA);
    last_block = tvb_get_guint8(tvb, *offset);
    *offset += 1;

    proto_tree_add_item(subtree, *dlms_hdr.block_number.p_id, tvb, *offset, 4, ENC_BIG_ENDIAN);
...
dlms_dissect_data_access_result(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gint *offset)
{
    ...
    item = proto_tree_add_item(tree, *dlms_hdr.data_access_result.p_id, tvb, *offset, 1, ENC_NA);
    result = tvb_get_guint8(tvb, *offset);
    *offset += 1;
    if (result != 0) {
