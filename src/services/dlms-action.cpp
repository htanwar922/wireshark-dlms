...
// Himanshu
void
dlms_dissect_ded_action_request(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gint offset)
{
    gint length = dlms_get_length(tvb, &offset);
    proto_tree *subtree = proto_tree_add_subtree(tree, tvb, offset, length, dlms_ett.ded_action_request, 0, "Action-Request (Ded-Ciphered)");
...
// Himanshu
void
dlms_dissect_ded_action_response(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, gint offset)
{
    gint length = dlms_get_length(tvb, &offset);
    proto_tree *subtree = proto_tree_add_subtree(tree, tvb, offset, length, dlms_ett.ded_action_response, 0, "Action-Response (Ded-Ciphered)");
